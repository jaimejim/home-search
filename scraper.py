...
def clean_text(text):
    """
    Clean extracted text by removing non-breaking spaces and formatting numbers correctly.
    """
    if text:
...
def extract_property_details(html_source):
    ...
    for dl in dl_elements:
        ...
        for row in rows:
            dt = row.find("dt", class_="info-table__title")
            dd = row.find("dd", class_="info-table__value")
            if dt and dd:
                title = dt.text.strip()
...
def format_property_data(json_data, html_source):
    ...
    return {
        "Address": clean_text(json_data.get("address", {}).get("streetAddress", "N/A")),
        "City": clean_text(json_data.get("address", {}).get("addressRegion", "N/A")),
        "Neighborhood": clean_text(json_data.get("address", {}).get("addressLocality", "N/A")),
        "Postal Code": clean_text(json_data.get("address", {}).get("postalCode", "N/A")),
        "Latitude": float(json_data.get("geo", {}).get("latitude", 0)),
        "Longitude": float(json_data.get("geo", {}).get("longitude", 0)),
        "Price (€)": clean_text(str(json_data.get("offers", {}).get("price", "N/A"))),
        "Size (m²)": clean_text(str(json_data.get("floorSize", {}).get("value", "N/A"))),
        "Condition": clean_text(json_data.get("itemCondition", {}).get("name", "N/A")),
        "Bedrooms": clean_text(str(json_data.get("numberOfRooms", "N/A"))),
        "Description": clean_text(json_data.get("description", "N/A")),
